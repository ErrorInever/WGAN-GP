...
def train_one_epoch(epoch, dataloader, gen, critic, opt_gen, opt_critic,
	...
	for batch_idx, (real_data, labels) in enumerate(dataloader):
		...
		for _ in range(cfg.CRITIC_ITERATIONS):
			...
			critic_real = critic(real_data, labels).reshape(-1)
			critic_fake = critic(fake_data, labels).reshape(-1)
			# gradient penalty
			grad_pen = gradient_penalty(critic, labels, real_data, fake_data, device)
			# loss function
		...
		gen_fake = critic(fake_data, labels).reshape(-1)
		...
		if batch_idx % freq == 0 and batch_idx > 0:
			static_fake_data = gen(static_noise, labels)
